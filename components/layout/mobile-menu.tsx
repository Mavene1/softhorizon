...
import { megaMenus, simpleLinks, navCta, isMegaMenuActive } from "@/components/layout/nav-config";
    ...
    <Sheet open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <SheetContent side="right" className="overflow-y-auto">
        ...
        <nav className="flex flex-col gap-1 px-4 pb-4" aria-label="Mobile navigation">
          {megaMenus.map((menu) => {
            const menuActive = isMegaMenuActive(menu, pathname);
            return (
              <Collapsible
                key={menu.label}
              ...
              >
                <CollapsibleTrigger
                  className={cn(
                    "flex w-full items-center justify-between rounded-lg px-3 py-2.5 text-sm font-medium hover:bg-accent transition-colors",
                    menuActive ? "text-primary" : "text-foreground"
                  )}
                >
                  {menu.label}
                  <DynamicIcon
                          ...
                          return (
                            <SheetClose key={item.label} asChild>
                              <Link
                                href={item.href}
                                className={cn(
                                  "flex items-center gap-3 rounded-lg px-2 py-2 text-sm transition-colors hover:bg-accent hover:text-foreground",
                                  itemActive ? "bg-accent text-primary" : "text-muted-foreground"
                                )}
                              >
